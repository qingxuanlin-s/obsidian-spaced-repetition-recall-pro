...
export class reviewResponseModal {
    ...
    private unloadBoundViews = new WeakSet<MarkdownView>();
    ...
    build() {
        ...
        if (view) {
            ...
            this.addKeysEvent();
            if (!this.unloadBoundViews.has(view)) {
                const originalOnUnload = view.onunload?.bind(view);
                view.onunload = () => {
                    this.close();
                    originalOnUnload?.();
                };
                this.unloadBoundViews.add(view);
            }
        }
