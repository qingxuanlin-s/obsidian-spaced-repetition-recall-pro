...
export default class SRPlugin extends Plugin {
    ...
    async onload(): Promise<void> {
        ...
        this.statusBar.setAttribute("aria-label", t("REVIEW_CARDS"));
        this.statusBar.setAttribute("aria-label-position", "top");
        this.statusBar.addEventListener("click", async () => {
            if (!this.syncLock) {
                await this.sync();
                this.openStatusBarReview();
            }
        });
    ...
    async reviewNextNote(deckKey: string): Promise<void> {
        ...
        if (MixQueSet.isDue() && deck.dueNotesCount > 0) {
            ...
            const path = item ? this.store.getFilePath(item) : null;
            const note =
                typeof path === "string" && path.length > 0
                    ? this.app.vault.getAbstractFileByPath(path)
                    : null;
            if (item != null && item.isTracked && note instanceof TFile) {
    ...
    private openStatusBarReview(): void {
        if (this.remainingDeckTree.getDistinctCardCount(CardListType.All, true) > 0) {
            if (this.data.settings.openViewInNewTab) {
                this.tabViewManager.openSRTabView(FlashcardReviewMode.Review);
            } else {
                this.openFlashcardModal(
