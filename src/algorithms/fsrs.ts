...
interface FsrsSettings {
    ...
    path_specific_max_intervals: { [path: string]: number };
    w: readonly number[];
    enable_fuzz: boolean;
...
export class FsrsAlgorithm extends SrsAlgorithm {
    ...
    /**
     * 根据文件路径获取对应的最大间隔
     * @param deckName 文件路径/卡组名
     * @returns 最大间隔（天）
     */
    getMaxIntervalForPath(deckName: string): number {
        if (!this.settings.path_specific_max_intervals) {
            return this.settings.maximum_interval;
        }

        // 最长前缀优先匹配（包含精确匹配）
        let matchedInterval: number | null = null;
        let matchedPathLength = -1;
        const paths = Object.keys(this.settings.path_specific_max_intervals);
        for (const path of paths) {
            if (deckName === path || deckName.startsWith(path + "/")) {
                if (path.length > matchedPathLength) {
                    matchedPathLength = path.length;
                    matchedInterval = this.settings.path_specific_max_intervals[path];
                }
            }
        }

        return matchedInterval ?? this.settings.maximum_interval;
    }
    ...
    calcAllOptsIntervals(item: RepetitionItem) {
        ...
        // 根据文件路径获取对应的最大间隔
        const maxInterval = this.getMaxIntervalForPath(item.deckName);
        const originalMaxInterval = this.fsrs.parameters.maximum_interval;
        
        // 临时设置最大间隔
        this.fsrs.parameters = { maximum_interval: maxInterval };
        
        const scheduling_cards = this.fsrs.repeat(card, now);
        
        // 恢复原始最大间隔
        this.fsrs.parameters = { maximum_interval: originalMaxInterval };
    ...
    displaySettings(
        ...
        new Setting(containerEl)
            .setName(t("PATH_SPECIFIC_MAX_INTERVALS"))
            .setDesc(t("PATH_SPECIFIC_MAX_INTERVALS_DESC"))
            .addTextArea((text) => {
                // 将对象转换为文本格式: "路径:天数"，每行一个
                const currentValue = Object.entries(this.settings.path_specific_max_intervals || {})
                    .map(([path, days]) => `${path}:${days}`)
                    .join("\n");
                
                text.setValue(currentValue)
                    .setPlaceholder("示例:\nFolder1:30\nFolder2/Subfolder:60")
                    .onChange((value) => {
                        applySettingsUpdate(async () => {
                            try {
                                const pathIntervals: { [path: string]: number } = {};
                                ...
                                for (const line of lines) {
                                    ...
                                    if (parts.length === 2) {
                                        ...
                                        const days = Number.parseInt(parts[1].trim());
                                        
                                        if (path && !isNaN(days) && days >= 1) {
                                            pathIntervals[path] = days;
                                        }
                                    }
                                }
                                
                                this.settings.path_specific_max_intervals = pathIntervals;
                                update(this.settings);
                                this.updateFsrsParams();
                            } catch (error) {
                                console.error("解析路径特定最大间隔配置时出错:", error);
                                new Notice(t("VALID_NUMBER_WARNING"));
                            }
                        });
                    });
            ...
            .addExtraButton((button) => {
                button
                    ...
                    .onClick(async () => {
                        applySettingsUpdate(async () => {
                            this.settings.path_specific_max_intervals = {};
                            update(this.settings, true);
                            this.updateFsrsParams();
                        });
                    });
