...
export default {
    ...
    GROUP_CONTRIBUTING: "参与贡献",
    CHECK_WIKI: '了解更多，请查看 <a href="${wikiUrl}">wiki</a>。',
    GITHUB_DISCUSSIONS:
        '欢迎访问 <a href="${discussionsUrl}">discussions</a> 参与问答、反馈和交流。',
    GITHUB_ISSUES:
        '如果你有功能建议或 Bug 反馈，请在 <a href="${issuesUrl}">这里提交 issue</a>。',
    GITHUB_ISSUES_MODIFIED_PLUGIN:
        '如果你在此修改版 sr 插件中遇到功能需求或 Bug，请在 <a href="${issuesUrl}">这里提交 issue</a>。',
    GITHUB_SOURCE_CODE:
        '项目源码可在 <a href="${githubProjectUrl}">GitHub</a> 查看。',
    CODE_CONTRIBUTION_INFO:
        '查看<a href="${codeContributionUrl}">这里</a>了解如何为插件贡献代码。',
    TRANSLATION_CONTRIBUTION_INFO:
        '查看<a href="${translationContributionUrl}">这里</a>了解如何为插件贡献翻译。',
    FOLDERS_TO_IGNORE: "忽略此文件夹",
    FOLDERS_TO_IGNORE_DESC:
        "在单独一行中输入文件夹路径或全局模式，例如 Templates/Scripts 或 .excalidraw.md。此设置对闪存卡和笔记都通用。",
    OBSIDIAN_INTEGRATION: "与 Obsidian 集成",
    FLASHCARDS: "卡片",
    ...
    BURY_SIBLINGS_TILL_NEXT_DAY_BY_NOTE_REVIEW: "按笔记复习时将关联卡片隐藏至下一天",
    ...
    PATH_SPECIFIC_MAX_INTERVALS: "路径特定最大间隔",
    PATH_SPECIFIC_MAX_INTERVALS_DESC: "为不同文件夹/路径设置不同的最大间隔时间。格式为‘路径:天数’，每行一个。例如：‘Folder1:30’表示Folder1及其子文件夹中的卡片最大间隔为30天。如果不设置，将使用上面的默认最大间隔。",
    ...
    DISPLAY_PARSER_DEBUG_INFO: "在开发者控制台中显示解析器调试信息",
    SCHEDULING: "排程",
    ...
    FSRS_ALGORITHM_DESC:
        '用于间隔重复的算法。更多信息请查看 <a href="https://github.com/open-spaced-repetition/ts-fsrs">FSRS 算法</a>。',
    ...
    UNTRACK_WITH_REVIEWTAG: "随复习标签取消追踪",
    ...
    CARD_BLOCK_ID: "卡片块 ID",
    CARD_BLOCK_ID_DESC:
        "使用卡片块 ID 替代行号与文本哈希。<br><b>开启后会在卡片文本后追加 block id；即使之后关闭，该 block id 也会保留在笔记中。</b>",
    CARD_BLOCK_ID_CONFIRM:
        "**开启后会在卡片文本后追加 block id；即使之后关闭，该 block id 也会保留在笔记中。**\n\n建议：开启前先备份你的库，或先在沙盒库中测试。\n\n开启该设置后，所有卡片后都会添加 block id。即便再次关闭，已添加的 block id 也会继续保留，不会自动删除。\n\n建议先**备份**你的笔记库，或在沙盒库中试用。",

    // mixQueueSetting.ts
    MIX_QUEUE: "混合队列",
    MIX_QUEUE_DESC:
        "复习时混合到期和新笔记。**第一个**滑块是总数量，第二个滑块是到期数量；新笔记数量 =（总数 - 到期数）。",

    // trackSetting.ts
    UNTRACK_WITH_REVIEWTAG_DESC:
        "当你在笔记中删除复习标签时，是否同步执行取消追踪，让该笔记不再进入复习<br><b>开启</b>：同步取消追踪；<br><b>关闭</b>：删除复习标签后，还需要手动再取消追踪一次（与旧版本一致）。",

    // dataLocation.ts
    DATA_LOCATION_PLUGIN_FOLDER: "在插件文件夹中",
    DATA_LOCATION_ROOT_FOLDER: "在库根目录中",
    DATA_LOCATION_SPECIFIED_FOLDER: "在下方指定的文件夹中",
    DATA_LOCATION_SAVE_ON_NOTE_FILE: "保存到笔记文件中",

    // locationSetting.ts
    POST_ISSUE_MODIFIED_PLUGIN:
        '如果你在这个带设置背景色的修改版插件中遇到问题，请提交 <a href="${issue_url}">issue</a>。',
